...
export default class RacingGame {
  ...
  private async 자동차등록() {
    ...
    const carNames = await RacingGame.carNames();
  ...
  private async 자동차경주() {
    const attemptCount = await RacingGame.attemptCount();
  ...
  private static async carNames() {
    const input = await InputView.readCarNames();
    return input.split(",");
  }

  private static async attemptCount() {
